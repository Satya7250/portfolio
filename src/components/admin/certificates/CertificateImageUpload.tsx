    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-dashed p-3">
        {value ? (
          ...
          <button
            type="button"
            className="text-muted-foreground hover:text-foreground flex min-h-40 w-full flex-col items-center justify-center gap-2 rounded-lg px-4 text-center transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
            disabled={disabled || isUploading}
            onClick={() => inputRef.current?.click()}
            aria-describedby="certificate-image-help"
          >
            {isUploading ? <Loader2 className="size-7 animate-spin" /> : <ImagePlus className="size-7" />}
